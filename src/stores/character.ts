...
export const useCharacterStore = defineStore('character', () => {
  ...
  const hpMax = computed(() =>
    10 + 5 * Math.round(((char.stats['COR'] || 0) + (char.stats['VOL'] || 0)) / 2),
  )
  const hpPct = computed(() => (hpMax.value ? Math.round((char.hp / hpMax.value) * 100) : 0))

  const statSum = computed(() => STAT_DEFS.reduce((t, d) => t + (char.stats[d.key] || 0), 0))

  const humanityMax = computed(() => (char.stats['EMP'] || 0) * 10)
  const humanityNow = computed(() => Math.max(0, Math.min(humanityMax.value, char.humanity || 0)))
  const humanityPct = computed(() =>
    humanityMax.value ? Math.round((humanityNow.value / humanityMax.value) * 100) : 0,
  )
  ...
  const idCode = computed(() => {
    const base = (char.alias || char.nom || 'V')
      ...
      .slice(0, 3)
      .padEnd(3, 'X')
    return 'NC-' + base + '-' + String((statSum.value * 7 + humanityMax.value) % 9000 + 1000)
  })
  ...
  function setSkill(name: string, delta: number) {
    char.skills[name] = Math.max(0, Math.min(10, (char.skills[name] || 0) + delta))
  }
  ...
  function setHumanity(delta: number) {
    char.humanity = Math.max(0, Math.min(humanityMax.value, (char.humanity || 0) + delta))
  }
  function setHumanityVal(val: string | number) {
    char.humanity = Math.max(0, Math.min(humanityMax.value, parseInt(String(val)) || 0))
  }

  function setEddies(val: string | number) {
    char.eddies = Math.max(0, parseInt(String(val).replace(/[^0-9]/g, '')) || 0)
  }
